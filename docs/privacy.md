...
Опубликованные дайджесты и связанные метрики хранятся 90 дней. Более старые
записи удаляются daily cleanup job.

В `usage_daily_stats` дополнительно сохраняются дневные агрегаты по расходам и
задержкам для каждой беседы. Эта таблица нужна для `/livanda-debug`, чтобы
статистика не зависела от хранения полных текстов дайджестов.
...
В приложении есть настраиваемые сроки хранения:

- `SUMMARY_HISTORY_RETENTION_DAYS` - тексты опубликованных дайджестов и
  диапазоны публикаций, по умолчанию 90 дней;
- `MESSAGE_RETENTION_DAYS` - сырые необработанные сообщения, по умолчанию 90
  дней.

- Необработанные сообщения хранятся до успешной публикации соответствующего
  дайджеста и фиксации выпуска в PostgreSQL либо до истечения
...
- В малоактивной беседе сообщения старше `MESSAGE_RETENTION_DAYS` удаляются
  daily cleanup job.
- Опубликованные дайджесты и метрики хранятся `SUMMARY_HISTORY_RETENTION_DAYS`.
- Дневные агрегаты usage также чистятся daily cleanup job по сроку
  `SUMMARY_HISTORY_RETENTION_DAYS`.
...
4. Удалить все данные беседы, заменив `2000000001` на нужный `peer_id`:
   ...
   DELETE FROM usage_daily_stats WHERE peer_id = 2000000001;
